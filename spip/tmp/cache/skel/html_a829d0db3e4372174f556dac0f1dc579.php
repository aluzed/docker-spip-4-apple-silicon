...
function BOUCLE_articleshtml_a829d0db3e4372174f556dac0f1dc579(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
...
quete_condition_postdates('articles.date',''));
	...
	$iter = Spip\Compilateur\Iterateur\Factory::create(
		"SQL",
		$command,
		array('squelettes-dist/inclure/recents.html','html_a829d0db3e4372174f556dac0f1dc579','_articles',6,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
...
'">
			' .
retablir_echappements_modeles(filtrer('image_graver',filtrer('image_reduire',quete_html_logo(quete_logo('id_article', 'ON', $Pile[$SP]['id_article'], $Pile[$SP]['id_rubrique']), '', (vider_url(urlencode_1738(generer_objet_url($Pile[$SP]['id_article'], 'article', '', '', true))))),'150','*'))) .
'
...
'</abbr>' .
(($t1 = strval(retablir_echappements_modeles(supprimer_tags(recuperer_fond('modeles/lesauteurs', array('objet'=>'article','id_objet' => $Pile[$SP]['id_article'],'id_article' => $Pile[$SP]['id_article']), array('trim'=>true, 'compil'=>array('squelettes-dist/inclure/recents.html','html_a829d0db3e4372174f556dac0f1dc579','_articles',7,$GLOBALS['spip_lang'])), '')))))!=='' ?
		((	', ' .
...
// Temps de compilation total: 0.745 ms
//

function html_a829d0db3e4372174f556dac0f1dc579($Cache, $Pile, $doublons = array(), $Numrows = array(), $SP = 0) {

	if (isset($Pile[0]["doublons"]) AND is_array($Pile[0]["doublons"]))
		$doublons = nettoyer_env_doublons($Pile[0]["doublons"]);

	$connect = '';
	$page = (($t1 = BOUCLE_articleshtml_a829d0db3e4372174f556dac0f1dc579($Cache, $Pile, $doublons, $Numrows, $SP))!=='' ?
		((	'
<div class="menu menu_articles" id="articles_recents">
	<h2 class="offscreen">' .
		...
		'</h2>
	' .
		retablir_echappements_modeles(filtre_pagination_dist($Numrows["_articles"]["grand_total"],
 		'_articles',
	...
	' .
		(($t3 = strval(retablir_echappements_modeles(filtre_pagination_dist($Numrows["_articles"]["grand_total"],
 		'_articles',
		isset($Pile[0]['debut_articles'])?$Pile[0]['debut_articles']:intval(_request('debut_articles')),
...
')) :
	...
	return analyse_resultat_skel('html_a829d0db3e4372174f556dac0f1dc579', $Cache, $page, 'squelettes-dist/inclure/recents.html');
}
